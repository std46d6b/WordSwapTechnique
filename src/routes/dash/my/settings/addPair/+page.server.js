...
export const actions = {
	addPair: async ({ cookies, request }) => {
		...
		if (!homeName?.length) {
			return { success: false, comment: 'invalid homeName' }
		}
		if (!homeCode?.length) {
			return { success: false, comment: 'invalid homeCode' }
		}
		if (!homeEmoji?.length) {
			return { success: false, comment: 'invalid homeEmoji' }
		}
		if (!goalName?.length) {
			return { success: false, comment: 'invalid goalName' }
		}
		if (!goalCode?.length) {
			return { success: false, comment: 'invalid goalCode' }
		}
		if (!goalEmoji?.length) {
			return { success: false, comment: 'invalid goalEmoji' }
		}
