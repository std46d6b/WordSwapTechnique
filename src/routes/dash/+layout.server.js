...
export async function load({ cookies }) {
    ...
    try {
        ...
        if (!decoded) { throw redirect(302, '/auth') }

        let userId = new ObjectId(decoded._id)
        if (!userId) { throw redirect(302, '/auth') }
        let dbUser = await userModel.findOne({ _id: userId, active: true })
        if (!dbUser) { throw redirect(302, '/auth') }

        decodedUserStore.update(currentState => { return { ...currentState, user: decoded, selectedLanguagePair: dbUser.selectedLanguagePair?.toString() } })
