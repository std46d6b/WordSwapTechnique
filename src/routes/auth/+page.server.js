...
export const actions = {
	reg: async ({ cookies, request }) => {
		...
		if (!email?.length) {
			return { success: false, comment: 'invalid email' }
		}
		if (!password?.length) {
			return { success: false, comment: 'password is too short' }
		}

		let user = await userModel.findOne({ email: email })

		if (user) {
			return { success: false, comment: 'email exists' }
		}
		...
		if (adminCheck) {
			...
			if (user) {
				return { success: false, comment: 'admin exists' }
			}
		...
		let token = jwt.sign({ _id: dbUser._id, admin: dbUser.admin, email: email }, JWT_TOKEN, {
			expiresIn: '30d'
		})
	...
	log: async ({ cookies, request }) => {
		...
		if (!password?.length) {
			return { success: false, comment: 'invalid password' }
		}

		let user = await userModel.findOne({ email: email, active: true })

		if (!user) {
			return { success: false, comment: 'user not found' }
		}
